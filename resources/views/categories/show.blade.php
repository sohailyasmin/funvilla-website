    ...
    <div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-md p-5 pb-6 ">
            <div class="grid sm:grid-cols-1 gap-x-8 gap-y-4">
                ...
                {{-- slug --}}
                <div class="input-area">
                    <label for="lccation" class="form-label font-bold">{{ __('Location') }}</label>
                    <input name="lccation" type="text" id="lccation" class="form-control"
                           value="{{ $category->location->name }}" disabled>
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>
                ...
                <div class="input-area">
                    ...
                    <input name="created_at" type="text" id="created_at" class="form-control"
                           value="{{ $category->created_at->format('Y-m-d h:i:s A') }}" disabled>
                </div>
            </div>
        </div>
    </div>
