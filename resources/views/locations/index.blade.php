        ...
        <div class="card">
            <header class=" card-header noborder">
                <div class="justify-end flex gap-3 items-center flex-wrap">
                    {{-- Create Button start--}}
                    {{-- @can('location create') --}}
                    <a class="btn inline-flex justify-center btn-dark rounded-[25px] items-center !p-2 !px-3" href="{{ route('locations.create') }}">
                    ...
                    <a class="btn inline-flex justify-center btn-dark rounded-[25px] items-center !p-2.5" href="{{ route('locations.index') }}">
                ...
                <div class="overflow-x-auto -mx-6">
                    <div class="inline-block min-w-full align-middle">
                        <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                ...
                                <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                    ...
                                    <tr class="border border-slate-100 dark:border-slate-900 relative">
                                        <td class="table-td sticky left-0"># {{ $location->id }}</td>
                                        <td class="table-td">
                                        ...
                                        <td class="table-td">{{ $location->created_at ? $location->created_at->diffForHumans() : '' }}</td>
                                        
                                        <td class="table-td">
                                            <div class="action-btns space-x-2 flex">
                                                {{-- show --}}
                                                {{-- @can('location show') --}}
                                                {{-- <a class="action-btn" href="{{ route('locations.show', $location) }}" x-data="{ tooltip: 'View' }" x-tooltip="tooltip">
                                                ...
                                                {{-- Edit --}}
                                                {{-- @can('location update') --}}
                                                <a class="action-btn" href="{{ route('locations.edit', ['location' => $location]) }}">
                                                    <iconify-icon icon="uil:edit"></iconify-icon>
                                                </a>
                                                {{-- @endcan --}}
                                                {{-- delete --}}
                                                {{-- @can('location delete') --}}
                                                <form id="deleteForm{{ $location->id }}" method="POST" action="{{ route('locations.destroy', $location) }}" class="cursor-pointer">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="action-btn" onclick="sweetAlertDelete(event, 'deleteForm{{ $location->id }}')" type="submit">
                            ...
                            <x-table-footer :per-page-route-name="'locations.index'" :data="$locations"  :filter="null" />
                        </div>
                    </div>
                </div>
            </div>
